...
flags.DEFINE_string('model', 'lenet', """Name of run""")
...
def init():
    if FLAGS.dataset == 'mnist':
        import mnist_seq
        dataset = mnist_seq.MNIST(FLAGS.L)
...
def main(_):
    ...
    def Hash(y):
        with tf.variable_scope('hash'):
            y1 = relu(linear(y, nC, 1000))
            z_logits = []
            for i in range(n_sequence):
                with tf.variable_scope('linear'+str(i)):
                    z_logits.append(linear(y1, 1000, 2))
            return z_logits
    ...
    def InvHash(z_logits):
         with tf.variable_scope('invhash'):
            ...
            with tf.variable_scope('linear1'):
                y1 = relu(linear(z_concat, 2*n_sequence, 1000))
            with tf.variable_scope('linear2'):
                y__logit = linear(y1, 1000, nC)
            return y__logit
    ...
    def TestHash():
        ...
        if ok:
            ...
            with open('cifar100_strings.pkl', 'wb') as fp:
    ...
    sess = create_session()
    saver = tf.train.import_meta_graph('cifar100_cnn/model-40000.meta')
    saver.restore(sess, tf.train.latest_checkpoint('cifar100_cnn/'))
...
#---------------------------------------------------------------------------------------------#
    ...
    z = []
    for i in range(n_sequence):
        z.append(tf.nn.softmax(zlogits[i]))
    ...
    train_writer = tf.summary.FileWriter('cifar100_rnn/train', graph=sess.graph)
    test_writer = tf.summary.FileWriter('cifar100_rnn/test')
    ...
    for e in range(n_epoch):
        ...
        for i in range(0, dataset.train['data'].shape[0], nB):
            ...
            if i % 10000 == 0:
                ...
                saver2.save(sess,'cifar100_rnn/model', i)
                train_writer.flush()
                test_writer.flush()
        ...
        if((e+1)%10==0):
            ...
            print('TESTING')
            print('----------------------------------------------')
            for j in range(0,dataset.test['data'].shape[0], nB):
                batch = dataset.test_batch(nB)
                ...
                a = sess.run([rnn_acc,summary_op] + z, feed_dict = feed_dict)
